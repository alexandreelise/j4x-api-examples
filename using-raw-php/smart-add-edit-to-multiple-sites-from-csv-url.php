...
$csvUrl = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTtV7Bnj-E3mwnBgXbkZlzS476aHVp6vtZ7qdI5vxlPUUqNe_85S3ozT5_gzNkBDih4dL1f8uDeeh_g/pub?gid=522567559&single=true&output=csv'; // For example: https://example.org/sample-data.csv';
if ($isLocal) {
    $localCsvFile = dirname(__DIR__) . '/media/data/sample-data.csv';
...
defined('IS_CLI') || define('IS_CLI', hash_equals(PHP_SAPI, 'cli'));
...
$csvReader = function (
    string $url,
    array $keys,
    callable $givenNested,
    callable $enqueueMessageCallable,
    int $isSilent = 1,
    array $lineRange = [],
    ...
    try {
        ...
        while (!$isFinished && !feof($resource)) {
            ...
            if (!$isExpanded || (count($lineRange) > 1) || $isEdgeCaseSingleLineInRange) {
                $currentCsvLineNumber += 1;
            ...
            try {
                ...
                if (($isExpanded && (count($lineRange) > 1)) && ($currentCsvLineNumber > $maxLineNumber)) {
                    ...
                    throw new DomainException(
                        sprintf('Processing of CSV file done. Last line processed was line %d', $currentCsvLineNumber),
                        200,
                    );
                }
                ...
                if ($encodedContent === false) {
                    throw new RuntimeException('Current line seem to be invalid', 422);
                                        ...
                                        $lineRange,
                                        true,
                                    )) || !$isExpanded)) && is_callable($handler))) {
...
$cpuCounter = function (): int {
    ...
    return (int)preg_match_all('/(processor)/', $procCpuInfo);
};
            ...
            function ($dataValue) use (
                ...
                try {
                    ...
                    if (!isset($pool[$rollingPoolIndex % $currentCpuCount]) || (!($pool[$rollingPoolIndex % $currentCpuCount] instanceof Fiber))) {
                        $pool[$rollingPoolIndex % $currentCpuCount] = new Fiber(
                            function (
                                string $givenHttpVerb,
                                string $endpoint,
                                string $dataString,
                                ...
                                int $timeout,
                                CurlHandle $transport,
                            ) use ($process, $combinedHttpResponse, $dataCurrentCSVline): void {
                                $combinedHttpResponse[$dataCurrentCSVline] ??= $process(
                                    $givenHttpVerb,
                                    $endpoint,
                                    $dataString,
                                    $headers,
                        ...
                        $combinedHttpResponse[$dataCurrentCSVline] ??= $pool[$rollingPoolIndex % $currentCpuCount]->start(
                            $pk ? 'PATCH' : 'POST',
                            $endpoint($baseUrl[$decodedDataString->tokenindex], $basePath, $pk),
                            $dataString,
                            $headers,
                            $timeout,
                            $curl,
                    ...
                    while (!$pool[$rollingPoolIndex % $currentCpuCount]->isTerminated()) {
                        $pool[$rollingPoolIndex % $currentCpuCount]->resume(
                            [
                                ($pk ? 'PATCH' : 'POST'),
                                $endpoint($baseUrl[$decodedDataString->tokenindex], $basePath, $pk),
                                $dataString,
                        ...
                        if (isset($decodedJsonOutput->errors) && !isset($storage[$dataCurrentCSVline])) {
                            ...
                            $storage[$dataCurrentCSVline] = [
                                'mightExists'       => isset($decodedJsonOutput->errors[0]->code) && ($decodedJsonOutput->errors[0]->code === 400),
                                'decodedDataString' => $decodedDataString,
                            ];
                        }
                        if (isset($decodedJsonOutput->data->attributes) && !isset($successfulCsvLines[$dataCurrentCSVline])) {
                            if ($silent == 1) {
    ...
    foreach ($storage as $dataCurrentCSVlineToRetry => $item) {
        ...
        try {
            if ($item['mightExists']) {
                ...
                $item['decodedDataString']->alias = sprintf(
                    '%s-%s',
                    $item['decodedDataString']->alias,
                ...
                if (!isset($poolRetry[$rollingPoolIndexRetry % $currentCpuCount]) || (!($poolRetry[$rollingPoolIndexRetry % $currentCpuCount] instanceof Fiber))) {
                    $poolRetry[$rollingPoolIndexRetry % $currentCpuCount] = new Fiber(
                        function (
                            string $givenHttpVerb,
                            string $givenEndpoint,
                            string $givenDataString,
                            array $givenHeaders,
                            int $givenTimeout,
                            CurlHandle $givenTransport,
                        ) use ($process, $combinedHttpResponse, $dataCurrentCSVlineToRetry): void {
                            $combinedHttpResponse[$dataCurrentCSVlineToRetry] ??= $process(
                                $givenHttpVerb,
                                $givenEndpoint,
                                $givenDataString,
                                $givenHeaders,
                            ...
                            $args                                             = Fiber::suspend(
                                $combinedHttpResponse[$dataCurrentCSVlineToRetry],
                            );
                            $combinedHttpResponse[$dataCurrentCSVlineToRetry] ??= $process(...$args);
                        },
                    );

                    $combinedHttpResponse[$dataCurrentCSVlineToRetry] ??= $poolRetry[$rollingPoolIndexRetry % $currentCpuCount]->start(
                        $pk ? 'PATCH' : 'POST',
                        $endpoint($baseUrl[$item['decodedDataString']->tokenindex], $basePath, $pk),
                        $dataString,
                        $headers,
                        $timeout,
                        $curlRetry,
                ...
                while (!$poolRetry[$rollingPoolIndexRetry % $currentCpuCount]->isTerminated()) {
                    $poolRetry[$rollingPoolIndexRetry % $currentCpuCount]->resume(
                        [
                            $pk ? 'PATCH' : 'POST',
                            $endpoint($baseUrl[$item['decodedDataString']->tokenindex], $basePath, $pk),
                            $dataString,
                    ...
                    $decodedJsonOutputRetry = json_decode(
                        $combinedHttpResponse[$dataCurrentCSVlineToRetry],
                        false,
                        512,
                        JSON_THROW_ON_ERROR,
                    ...
                    if (isset($decodedJsonOutputRetry->data->attributes) && !isset($successfulCsvLines[$dataCurrentCSVlineToRetry])) {
                        if ($silent == 1) {
                            $successfulCsvLines[$dataCurrentCSVlineToRetry] = sprintf(
                                '%s Deployed to: %s, CSV Line: %d, id: %d,  created: %s, title: %s, alias: %s%s%s',
                                ANSI_COLOR_GREEN,
            ...
            if ($silent == 1) {
                $failedCsvLines[$dataCurrentCSVlineToRetry] = sprintf(
                    '%s Error message: %s, Error code line: %d, Error CSV Line: %d%s%s',
                    ANSI_COLOR_RED,
                    $e->getMessage(),
                    $e->getLine(),
